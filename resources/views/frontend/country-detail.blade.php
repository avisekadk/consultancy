...
<h1 class="text-3xl md:text-4xl font-bold text-center">Study in {{ $country->title }}</h1>
</div>
</section>
...
@if($country->image)
<img src="{{ asset('storage/' . $country->image) }}" alt="{{ $country->title }}" class="w-full h-[400px] object-cover rounded-2xl shadow-lg mb-6">
@endif
...
<p class="text-base text-gray-700 leading-relaxed font-semibold">{{ $country->short_description }}</p>
</div>
</div>
@endif

<div class="mb-6">
<h2 class="text-2xl font-bold text-gray-900 mb-4">About {{ $country->title }}</h2>
...
@if($country->image_1)
<div class="mb-6">
<img src="{{ asset('storage/' . $country->image_1) }}" alt="{{ $country->title }}" class="w-full h-[350px] object-cover rounded-xl shadow-md">
</div>
@endif

<!-- CTA Box -->
<div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-6">
<h3 class="text-xl font-bold text-gray-900 mb-3">Interested in Studying in {{ $country->title }}?</h3>
<p class="text-gray-600 mb-4 text-sm">Connect with our expert counselors for personalized guidance on universities, courses, and visa process.</p>
...
Start Your Application <i class="fas fa-arrow-right ml-2 text-xs"></i>
</a>
</div>
</div>
...
<h3 class="text-xl font-bold text-gray-900 mb-4">Other Destinations</h3>
<div class="space-y-4">
...
<a href="/countries/{{ $related->slug }}" class="inline-flex items-center text-purple-600 font-semibold text-xs group-hover:text-purple-700">
Explore <i class="fas fa-arrow-right ml-1 text-xs group-hover:translate-x-1 transition-transform duration-300"></i>
</a>
</div>
@if(!$loop->last)
