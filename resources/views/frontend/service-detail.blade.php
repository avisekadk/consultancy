...
<h1 class="text-4xl md:text-5xl font-bold text-center">{{ $service->title }}</h1>
</div>
</section>
...
@if($service->image)
<img src="{{ asset('storage/' . $service->image) }}" alt="{{ $service->title }}" class="w-full rounded-3xl shadow-2xl mb-8">
@endif
...
<p class="text-lg text-gray-700 leading-relaxed">{{ $service->short_description }}</p>
</div>
</div>
@endif

<div class="mb-8">
<h2 class="text-3xl font-bold text-gray-900 mb-6">Service Details</h2>
...
@if($service->image_1)
<div class="mb-8">
<img src="{{ asset('storage/' . $service->image_1) }}" alt="{{ $service->title }}" class="w-full rounded-2xl shadow-lg">
</div>
@endif

<!-- CTA Box -->
<div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-8">
<h3 class="text-2xl font-bold text-gray-900 mb-4">Need This Service?</h3>
<p class="text-gray-600 mb-6">Contact our experts to learn more about how we can help you.</p>
...
<h3 class="text-2xl font-bold text-gray-900 mb-6">Other Services</h3>
<div class="space-y-6">
...
<a href="/services/{{ $related->slug }}" class="inline-flex items-center text-purple-600 font-semibold text-sm group-hover:text-purple-700">
